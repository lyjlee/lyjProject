...
//    게시글 등록
    @Insert("INSERT INTO mydb.board(board_title, board_des, user_name, board_submittedAt)" +
...
//    댓글 등록
    @Insert("INSERT INTO mydb.REPLY(board_no, user_name, reply_content, reply_submittedAt)" +
...
//     페이징 위한 최신댓글순 정렬 + row_number 받아오기
    ...
    //    댓글 수정
    @Update("UPDATE mydb.REPLY SET reply_content=#{content} WHERE reply_no=#{replyNo} and board_no=#{no} and user_name=#{userId}")
    void submitEditReply(String content, int no, String userId, int replyNo);

//    댓글 삭제
    @Delete("DELETE FROM mydb.REPLY WHERE reply_no=#{replyNo} and board_no=#{no} and user_name=#{userId}")
    void removeReply(int no, String userId, int replyNo);

//    대댓글 존재 댓글 삭제처리
    @Update("UPDATE mydb.REPLY SET reply_content='삭제된 댓글입니다', user_name='null' WHERE reply_no=#{replyNo} and board_no=#{no}")
    void removeTempRReply(int no, int replyNo);


    //    대댓글 등록
    @Insert("INSERT INTO mydb.REPLY(board_no, user_name, reply_content, reply_submittedAt, reply_level, reply_parent_no)" +
